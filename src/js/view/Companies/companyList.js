    ...
    const markup = `
        <div style="border: 5px brown solid;border-radius: 10px; padding: 10px; width: 500px" class="company" id="company_69312">
            ...
            <div style="display:block; margin: 30px ">
                ${company.keySkills.slice(0,3).map(tag =>  `<a href="#search-${tag}"><div style="${Utility.tagStyle}">${tag}</div></a>`).join('')}
            </div>
            <div style="display:block; margin: 30px ">
                ${company.keySkills.slice(3,10).map(tag => `<a href="#search-${tag}"><div style="${Utility.tagStyle}">${tag}</div></a>`).join('')}
            </div>
