...
const EachCompany = company => {
    const markup = `
        <div style="border: 5px brown solid;border-radius: 10px; padding: 10px; width: 500px" class="company" id="company_69312">
            <a style="color:red;font-size: 30px" href="companies/${Utility.getCompanyCodeFromUrl(company.url)}" >
            ...
            <div style="display:block; margin: 30px ">
                ${company.keySkills.slice(0,3).map(tag =>  `<a href="http://localhost:8080/search-${tag}"><div style="${Utility.tagStyle}">${tag}</div></a>`).join('')}
            </div>
            <div style="display:block; margin: 30px ">
                ${company.keySkills.slice(3,10).map(tag => `<a href="http://localhost:8080/search-${tag}"><div style="${Utility.tagStyle}">${tag}</div></a>`).join('')}
            </div>
            <a style="color:red;font-size:20px;float:right" href="companies/${Utility.getCompanyCodeFromUrl(company.url)}">Detail >>> </a>
        </div>
      `; 
    return markup;
};
